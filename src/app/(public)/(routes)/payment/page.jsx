...
import { BreadcrumbItem, Breadcrumbs, Button, Card, CardBody, CardHeader, Divider, Spinner } from "@heroui/react";
...
const PaymentVerifyPage = () => {
	...
	const orderId = orderData?.id.split("/").pop();
	...
	const handleVerifyPayment = async () => {
		try {
			...
			if (response.RspCode === "00") {
				...
				setCart([]);
			} else if (response.RspCode === "24" || responseCode === "24") {
				setStatus("customer_cancel");
				setError(response.orderData.message);
			}
				...
				<div className="flex flex-col gap-10 items-center">
						...
						<>
							<div className="flex flex-col items-center">
								...
								<p className="text-center text-md text-gray-500 mt-2">
									Đơn hàng của bạn đã được xác nhận. Chúng tôi sẽ liên hệ với bạn trong thời gian sớm
									nhất.
								</p>
							</div>
					...
					{status === "customer_cancel" && (
						<>
							...
							<p className="text-lg font-bold text-red-500">Thanh toán không thành công</p>
							<p className="text-gray-600">Bạn đã hủy thanh toán. Vui lòng đặt hàng và thanh toán lại.</p>
